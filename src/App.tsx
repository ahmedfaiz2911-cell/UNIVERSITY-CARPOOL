...
const RideCard = ({ ride, onRequestRide }: { ride: Ride; onRequestRide: (rideId: string) => void }) => (
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl p-6 w-full max-w-md">
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); onAuth(authMode); }}>
          {authMode === 'register' && (
            <div>
...
const CreateRideForm = ({ 
  ...
  setCurrentView: (view: 'home' | 'browse' | 'create' | 'profile') => void;
  setShowAuthModal: (show: boolean) => void;
}) => {
  // Individual state variables for each form field (prevents re-render issues)
  ...
  <div>
    ...
    {ridesLoading ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {rides.map(ride => (
          <RideCard key={ride.id} ride={ride} onRequestRide={onRequestRide} />
        ))}
      </div>
    )}
  </div>
...
const HomePage = ({ 
  ...
  onNavigate: (view: 'home' | 'browse' | 'create' | 'profile') => void 
}) => {
  const [homeSearchFrom, setHomeSearchFrom] = useState('');
  const [homeSearchTo, setHomeSearchTo] = useState('');
  const [homeSearchDate, setHomeSearchDate] = useState('');
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                <input 
                  ...
                  onChange={(e) => setHomeSearchFrom(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                />
              </div>
              <div>
                <input 
                  ...
                  placeholder="To"
                  value={homeSearchTo}
                  onChange={(e) => setHomeSearchTo(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                />
              </div>
              <div>
                <input 
                  type="date"
                  value={homeSearchDate}
                  onChange={(e) => setHomeSearchDate(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                />
              </div>
              <button 
